...
const Contact = () => {
  ...
  // Handle Spline loading and errors
  useEffect(() => {
    const handleSplineError = () => {
      setSplineError(true);
      console.warn('Spline 3D model failed to load, continuing without 3D background');
    };

    // Add event listeners for Spline component
    const splineElement = document.querySelector('spline-viewer');
    if (splineElement) {
      splineElement.addEventListener('error', handleSplineError);
    }

    // Cleanup
    return () => {
      if (splineElement) {
        splineElement.removeEventListener('error', handleSplineError);
      }
    };
  }, []);
  ...
  const validateForm = () => {
    ...
    if (!formData.studentName.trim()) {
      ...
      newErrors.studentName = 'Please enter a valid name (letters, spaces, hyphens only)';
    }

    if (!formData.parentName.trim()) {
      newErrors.parentName = 'Parent name is required';
    } else if (!validateName(formData.parentName)) {
      newErrors.parentName = 'Please enter a valid name (letters, spaces, hyphens only)';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!submissionResult.success) {
        throw new Error(submissionResult.error || submissionResult.details || 'Failed to submit form');
      }
      
      // Form submitted successfully
      
      // Show different success messages based on submission method
      const successMessage = submissionResult.method === 'google_forms' 
  ...
  const handleInputChange = (e) => {
    ...
    if (sanitizedValue.toLowerCase().includes('<script') || 
  ...
  const getInputClass = (fieldName) => {
    const baseClass = "w-full px-3 sm:px-3.5 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm md:text-[15px] rounded-lg bg-white/15 border text-white focus:outline-none focus:ring-2 placeholder-white/70";
    const errorClass = errors[fieldName] ? "border-red-500 focus:ring-red-500" : "border-white/20 focus:ring-blue-500";
    return `${baseClass} ${errorClass}`;
  };

  // Helper function to get select class with proper dropdown styling
  const getSelectClass = (fieldName) => {
    const baseClass = "w-full px-3 sm:px-3.5 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm md:text-[15px] rounded-lg bg-white/15 border text-white focus:outline-none focus:ring-2 appearance-none cursor-pointer";
    const errorClass = errors[fieldName] ? "border-red-500 focus:ring-red-500" : "border-white/20 focus:ring-blue-500";
    const selectSpecific = "[&>option]:bg-gray-800 [&>option]:text-white [&>option]:py-2 [&>option]:px-3";
  ...
  return (
    ...
    <section id="contact" className="min-h-screen relative overflow-hidden bg-black pt-12 sm:pt-16 md:pt-20 pb-8 sm:pb-10">
      ...
      <div className="absolute inset-0 hidden lg:block z-5">
         <div className="absolute right-0 top-0 w-3/5 h-full overflow-visible flex items-center justify-center">
           {!splineError && (
              <spline-viewer 
                url="https://prod.spline.design/kKyWeZz85xetpdcW/scene.splinecode"
                className="w-full h-full pointer-events-none scale-90"
                style={{ background: 'transparent', transform: 'translateX(8%) translateY(0%)' }}
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           >
             <div className="bg-white/15 md:bg-white/10 md:backdrop-blur-lg rounded-2xl border border-white/20 p-4 sm:p-6 md:p-8 lg:p-6 xl:p-8 shadow-2xl">
               ...
               <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-5 items-start">
                 <motion.div
                 ...
                 >
                   ...
                   <div className="relative">
                     <select
                       ...
                       required
                     >
                       <option value="" className="bg-gray-800 text-white">Select Grade</option>
                       <option value="Grade 5" className="bg-gray-800 text-white hover:bg-gray-700">Grade 5</option>
                       <option value="Grade 6" className="bg-gray-800 text-white hover:bg-gray-700">Grade 6</option>
                       <option value="Grade 7" className="bg-gray-800 text-white hover:bg-gray-700">Grade 7</option>
                       <option value="Grade 8" className="bg-gray-800 text-white hover:bg-gray-700">Grade 8</option>
                       <option value="Grade 9" className="bg-gray-800 text-white hover:bg-gray-700">Grade 9</option>
                       <option value="Grade 10" className="bg-gray-800 text-white hover:bg-gray-700">Grade 10</option>
                       <option value="Grade 11" className="bg-gray-800 text-white hover:bg-gray-700">Grade 11</option>
                       <option value="Grade 12" className="bg-gray-800 text-white hover:bg-gray-700">Grade 12</option>
                     </select>
                 ...
                 >
                   ...
                   <div className="relative">
                     ...
                     >
                       ...
                       <option value="Mathematics" className="bg-gray-800 text-white hover:bg-gray-700">Mathematics</option>
                       <option value="Science" className="bg-gray-800 text-white hover:bg-gray-700">Science</option>
                       <option value="English" className="bg-gray-800 text-white hover:bg-gray-700">English</option>
                       <option value="Coding" className="bg-gray-800 text-white hover:bg-gray-700">Coding</option>
                       <option value="Multiple Subjects" className="bg-gray-800 text-white hover:bg-gray-700">Multiple Subjects</option>
                     </select>
                     <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                       <svg className="w-4 h-4 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                 ...
                 >
                   ...
                   >
                     ...
                     <span>{isSubmitting ? 'Submitting...' : 'Book Free Demo Class'}</span>
                   </button>
                 </motion.div>
               </form>
             </div>
